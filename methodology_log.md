...
- `marginfi_liquidation.v1` row content, post-2026-05-03 amendment after IDL pre-flight:
  ...
  - From outer-tx token-balance changes (`meta.{pre,post}TokenBalances` synthesized into `ParsedTx::account_data`): `asset_amount_seized` (native u64) is the liquidator's net positive delta in `asset_mint` for that tx; `insurance_fund_fee_paid` (native u64) is the matching positive delta on the bank's `insurance_vault_authority` PDA, looked up from the `BankRegistry` built off `marginfi_reserve.v1` snapshots (PDA derived in-process from the `insurance_vault_authority_bump` byte at absolute account offset 179 via `Pubkey::create_program_address(&[b"insurance_vault_auth", bank, &[bump]], &MARGINFI_PROGRAM_ID)`; verified 100% derivation rate against the live 422-bank canonical snapshot, with bumps clustering at 255/254/253 per Anchor's `find_program_address` convention).
