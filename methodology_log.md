...
- Cycling-fire model (not KeepAlive daemon): each fire opens the subscription, drains for `--duration-secs` (default 55s to fit a 60s manifest tick), then exits cleanly. Reuses the existing per-manifest `runner-tick` infrastructure; the alternative — a long-running KeepAlive daemon — was rejected to avoid bespoke supervision logic.
