...
| M3.6 | Launchd Phase B migration | partial 2026-05-02 — `pyth-tape` migrated under per-manifest tick mode (`tick --only`/`--skip`, per-manifest state files, dedicated `runner-pyth-tape.plist`). Remaining Phase B plists (`backed-nav-strikes`, `cex-stock-perp-tape`, `kamino-scope-tape`, `v5-tape`, `chainlink-reports`) follow the same pattern in future passes | M3.5 |
