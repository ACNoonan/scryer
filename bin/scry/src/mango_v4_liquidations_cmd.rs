//! `scry solana mango-v4-liquidations` — Mango v4 liquidation panel.
//!
//! Same pipeline as Drift liquidations: sig-paginate via proxy →
//! parseTransactions (Helius) or proxy-routed getTransaction → IX
//! decode → write per-day no-key partitions.
...
use scryer_fetch_solana::mango_v4_liquidations::{extract_liquidations, MANGO_V4_PROGRAM};
...
use scryer_schema::mango_v4_liquidation::v1 as schema;
use scryer_schema::Meta;
use scryer_store::{venue, Dataset};

#[derive(Parser, Debug)]
pub struct MangoV4LiquidationsArgs {
    #[arg(long, default_value = "http://127.0.0.1:8899/rpc")]
    proxy_url: String,
    #[arg(long, env = "HELIUS_API_KEY", default_value = "")]
    helius_api_key: String,
    /// Proxy-routed `getTransaction` instead of Helius
    /// `parseTransactions`. Slower but quota-resilient.
    #[arg(long)]
...
pub async fn run_mango_v4_liquidations(args: MangoV4LiquidationsArgs) -> Result<()> {
    ...
    tracing::info!(
        ...
        "stage 1: paginating signatures against Mango v4 program"
    );
    ...
    if sigs.is_empty() {
        println!("mango_v4_liquidations: rows_added=0 (empty window)");
        return Ok(());
    }
    ...
    if rows.is_empty() {
        println!(
            "mango_v4_liquidations: rows_added=0 sigs_scanned={} (no liquidations decoded)",
            sig_strs.len()
        );
        return Ok(());
    }
    ...
    let stats = ds
        ...
        .context("Dataset::write mango_v4_liquidations")?;
    ...
    println!(
        "mango_v4_liquidations: rows_added={} rows_deduped={} partitions_written={} sigs_scanned={} per_type={:?}",
        stats.rows_added,
        stats.rows_deduped,
