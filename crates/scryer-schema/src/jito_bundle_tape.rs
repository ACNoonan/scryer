...
/// v2 namespace migration of [`v1`]. **Identical row shape** — same
/// columns, same types, same `_dedup_key`. Two things change:
///
/// 1. The schema id moves to the v2 four-part namespace
///    (`solana.jito.bundle_tape.v2`), which relocates the dataset root
///    to `dataset/solana.jito/bundle_tape/v2/`.
/// 2. That new root is written at **hourly** granularity, per the
///    "High-Volume Tape Partition Granularity — 2026-08-03" methodology
///    entry. v1's daily root at `dataset/jito/bundle_tape/v1/` stays
///    frozen and readable, following the `earnings.v1→v2` precedent.
///
/// The row code is duplicated rather than shared with [`v1`] because v1
/// is locked; a shipped major is an immutable snapshot in this repo, so
/// parameterising v1's encoder to serve both is not available.
pub mod v2 {
    use std::sync::Arc;
    ...
    pub const SCHEMA_VERSION: &str = "solana.jito.bundle_tape.v2";

    /// See [`super::v1::BundleLanding`] — field-for-field identical. The
    /// on-chain heuristic, `bundle_id` synthesis, and every limitation in
    /// the module doc-block carry over unchanged.
    #[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
    pub struct BundleLanding {
        pub slot: u64,
        pub block_time: i64,
        ...
        pub lead_tx_sig: String,
        /// Comma-joined ordered base58 sigs in the bundle group;
        /// the first entry equals `lead_tx_sig`.
        pub tx_sigs: String,
        pub tip_lamports: i64,
        /// Which of the 8 Jito tip pubkeys received the tip transfer.
        pub tip_account: String,
    ...
    impl BundleLanding {
        /// **Deliberately identical to v1's key**, including the
        /// `jito_bundle_tape:` prefix rather than a `solana.jito...`
        /// one. Two reasons, both load-bearing:
        ///
        /// - Identity is unchanged by a namespace or granularity move,
        ///   matching the `earnings.v1→v2` lock ("timing is an
        ///   attribute, not identity"). A stable key is what lets rows
        ///   be lifted from v1 into v2 without duplicating.
        /// - The leading `{slot}` is what makes hourly partitioning
        ///   safe: a slot maps to exactly one instant, so two rows
        ///   sharing a key can never land in different hour partitions.
        ///   Re-prefixing would not break that, but rewriting the key
        ///   shape at all would invite someone to drop the slot.
        pub fn dedup_key(&self) -> String {
            format!("jito_bundle_tape:{}:{}", self.slot, self.lead_tx_sig)
        }
        ...
        /// Helper: return tx_sigs as a Vec<&str>. Empty `tx_sigs`
        /// yields an empty Vec; bundles always have ≥1 tx by
        /// definition so empty in practice indicates a malformed row.
        pub fn tx_sigs_iter(&self) -> Vec<&str> {
            if self.tx_sigs.is_empty() {
                Vec::new()
            } else {
    ...
    mod tests {
        ...
        #[test]
        fn schema_version_constant_is_correct() {
            assert_eq!(SCHEMA_VERSION, "solana.jito.bundle_tape.v2");
        }

        /// The v2 key must stay byte-identical to v1's so rows lift
        /// across the namespace boundary without duplicating. If this
        /// ever diverges, a migration would double every row.
        #[test]
        fn dedup_key_is_byte_identical_to_v1() {
            let v2_row = sample(415_581_004, "sig_a");
            let v1_row = super::super::v1::BundleLanding {
                slot: 415_581_004,
                block_time: 1_777_300_000,
                bundle_id: "415581004:sig_a".to_string(),
                lead_tx_sig: "sig_a".to_string(),
                tx_sigs: "sig_a,sig_b,sig_c".to_string(),
                tip_lamports: 50_000,
                tip_account: TIP_ACCOUNT_1.to_string(),
                leader_pubkey: "LeaderPubkey1".to_string(),
                meta: Meta::new(
                    super::super::v1::SCHEMA_VERSION,
                    1_777_300_000,
                    "rpc:getBlock",
                ),
            };
            assert_eq!(v2_row.dedup_key(), v1_row.dedup_key());
            assert_eq!(v2_row.dedup_key(), "jito_bundle_tape:415581004:sig_a");
        }
        ...
        /// A v1-stamped row must not decode as v2. Without this, a
        /// mis-pointed reader could silently mix the frozen daily root
        /// into the hourly one.
        #[test]
        fn rejects_v1_schema_version_on_decode() {
            let mut row = sample(1, "sig_a");
            row.meta.schema_version = "jito_bundle_tape.v1".to_string();
            let batch = to_record_batch(&[row]).expect("encode");
            let err = from_record_batch(&batch).unwrap_err();
            assert!(matches!(err, FromArrowError::SchemaVersionMismatch { .. }));
        }
